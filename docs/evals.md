...
```bash
mise install                 # installs apm, jq, shellcheck, bats, and opencode (pinned)
...
# One skill
./scripts/test-triggers.sh kerpo-tdd          # runs=3 by default
./scripts/run-evals.sh     kerpo-tdd 1        # iteration 1
```
...
EVAL_MODEL=opencode/mimo-v2.5-free \
  ./scripts/test-triggers.sh kerpo-gha-workflow-create
```
...
Free models differ a lot in how sharply they judge a near-miss, and they are
retired or rate-limited over time. Keep `EVAL_MODEL` overridable and re-baseline
per skill and per model — do not compare a free-model run against older
Claude-haiku numbers.
...
- **Skill isolation** — `permission.skill` denies `*` and (for the
  *with skill* case) allows exactly the skill under test. Global and sibling
  skills are hidden, so `without_skill` really has no skill to load. The skill
  itself is copied into `.opencode/skills/<name>/`.
...
- **Model-dependent numbers.** Trigger/output rates belong to the model that
  produced them. Re-baseline per skill and per model.
- **Free models change.** They are "for a limited time" and can be retired or
  ...
  a model disappears.
- **Privacy.** Free Zen models may log prompts and, for some, use them for
  training. Skill descriptions and eval prompts are package-public; do not put
  secrets or customer data in them.
- **Speed.** Some free models are minutes-slow. `EVAL_TIMEOUT` bounds each call
  (exit code 124).
...
- **Summary** — each run writes a Markdown summary (model, per-skill
  ...
  `scripts/summarize-triggers.sh`.

It is deliberately separate from the PR **CI** workflow: a free model is slow
and near-miss noise is high, so a required PR check would be flaky. To gate PRs
on changed skills later, add a `pull_request` trigger that diffs `.apm/skills/`
and runs `./scripts/test-triggers.sh <skill> 1` for each touched skill.
